...
auto BufferPoolManager::CheckedWritePage(page_id_t page_id, AccessType access_type) -> std::optional<WritePageGuard> {
  ...
  if (page_table_.count(page_id)) {
    ...
    reqs.emplace_back(DiskRequest{false, frames_[fid]->GetDataMut(), page_id, std::move(promise)});
    disk_scheduler_->Schedule(reqs);
    future.get();  // 等待数据从磁盘读入
    page_table_[page_id] = fid;
    frames_[fid]->page_id_ = page_id;
...
auto BufferPoolManager::GetAvailableFrame() -> frame_id_t {
  ...
  if (!free_frames_.empty()) {
    ...
    auto evicted_fid = replacer_->Evict();
    if (!evicted_fid.has_value()) {
      return -1;
    }
    fid = *evicted_fid;
    auto evict_frame = frames_[fid];
    if (evict_frame->is_dirty_) {
      // 驱逐前必须同步写回脏页
      SyncFlush(fid, evict_frame->page_id_);
    }
    // 从页面表中移除旧页面的映射
...
/**
 * @brief 内部辅助函数：同步刷新特定帧到磁盘。
 */
void BufferPoolManager::SyncFlush(frame_id_t frame_id, page_id_t page_id) {
  auto promise = disk_scheduler_->CreatePromise();
  auto future = promise.get_future();
  DiskRequest req{true, frames_[frame_id]->GetDataMut(), page_id, std::move(promise)};
  std::vector<DiskRequest> reqs;
